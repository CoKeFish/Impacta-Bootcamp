...
COMMENT ON TABLE users IS 'Perfiles de usuarios de la plataforma';
COMMENT ON COLUMN users.wallet_address IS 'Clave pública de Stellar - vínculo con on-chain';
...
COMMENT ON TABLE trips IS 'Metadata de viajes grupales';
COMMENT ON COLUMN trips.contract_trip_id IS 'ID del viaje en el contrato multi-escrow de Soroban';
COMMENT ON COLUMN trips.status IS 'Cache del estado on-chain: draft/funding/completed/cancelled/released';
...
COMMENT ON TABLE trip_participants IS 'Participantes de cada viaje';
COMMENT ON COLUMN trip_participants.contributed_amount IS 'Cache del balance on-chain en XLM';
...
COMMENT ON TABLE transactions IS 'Índice de transacciones on-chain para historial y queries';
COMMENT ON COLUMN transactions.event_data IS 'Payload JSON del evento Soroban indexado';
...
COMMENT ON TABLE partners IS 'Empresas asociadas que ofrecen descuentos';
...
COMMENT ON TABLE trip_offers IS 'Ofertas de partners específicas para cada viaje';
...
COMMENT ON TABLE images IS 'Metadata de imágenes - archivos en MinIO/Supabase Storage';
...
INSERT INTO users (wallet_address, username) VALUES
    ('GBZXN7PIRZGNMHGA7MUUUF4GWPY5AYPV6LY4UV2GL6VJGIQRXFDNMADI', 'demo_user');

-- Partner de ejemplo
INSERT INTO partners (name, category, discount_percent, description) VALUES
    ('Hotel Demo', 'hotel', 15.00, 'Hotel de prueba para desarrollo');
